...
export function HeroSection({ eyebrow, title, description, supportLine, mockup }: HeroSectionProps) {
  return (
    ...
    <section id="hero" className="homeHero relative" data-reveal>
      ...
      />
      <div className="relative mx-auto grid w-full max-w-7xl gap-8 px-4 pb-12 pt-10 sm:px-6 md:pt-14 lg:grid-cols-[1.08fr_0.92fr] lg:items-center lg:px-8">
        <div className="heroCopy space-y-5">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-data-blue">{eyebrow}</p>
          <h1 className="text-balance text-4xl font-semibold leading-tight text-text sm:text-5xl lg:text-6xl">
            {title}
          </h1>
          <p className="lede text-pretty max-w-2xl text-base text-muted md:text-lg">{description}</p>
